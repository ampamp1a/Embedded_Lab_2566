...
void IRAM_ATTR onTimer_0() {
    if (state_timer_0 == LOW){
      timeAlarmWrite(timer_0, 20,true);
      digitalWrite(digital_PIN_T0,state_timer_0);
      state_timer_0 = HIGH;
    }
    else{
      timeAlarmWrite(timer_0, 80,true);
      digitalWrite(digital_PIN_T0,state_timer_0);
      state_timer_0 = LOW;      
    }
}
...
void setup() {
    ...
    timer_0 = timerBegin(0,80,true); //timerBegin(0 is timer number 0, 80 is prescalers divider, true is count-up timer)
    ...
    timeAlarmWrite(timer_0, 20000, true); //timeAlarmWrite(timer mode, 20000 is value to generate event, true is enable autoreload)
